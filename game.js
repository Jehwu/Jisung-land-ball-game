...
canvas.width = size;
canvas.height = size + 140; // 도네이션 텍스트 공간 확보
...
const characters = [
    ...
    { id: 7, name: "🛡️ 김민재", nickname: "민재", class: "tanker", color: "#3498db", maxHp: 275, radius: 24, speed: 1.3, bumpDmg: 0 },
    { id: 8, name: "✨ 차은우지성", nickname: "차은우지성", class: "util", color: "#ffb6c1", maxHp: 200, radius: 20, speed: 1.5, bumpDmg: 0 },
    { id: 9, name: "🔴 버튜버 김티비", nickname: "V티비", class: "util", color: "#ff9ff3", maxHp: 200, radius: 21, speed: 1.5, bumpDmg: 0 }
];

const dictionaryData = [
    { nickname: "민채", color: "#ff4757", name: "🛡️ 김민채", class: "tanker", hpStat: 95, atkStat: 50, spdStat: 40, desc: "패시브: 받는 피해 10% 감소 (분노 시 30% 감면 및 이속 100% 증가) 🛡️<br>스킬 1: 먹기 - 장판에 닿은 대상을 삼켜 피해.<br>스킬 2: 대지 강타 - 광역 지진 슬로우." },
    { nickname: "병은", color: "#2ed573", name: "⚡ 공병은", class: "dealer", hpStat: 55, atkStat: 75, spdStat: 75, desc: "스킬 1: 사이먼도미닉 / 15 / 3초 / 감전<br>스킬 2: 사이먼 앤 도미닉 봄 / 15 / 3.5초 / 체력 회복 및 감전" },
    { nickname: "지성", color: "#f1c40f", name: "👑 박지성", class: "dealer", hpStat: 65, atkStat: 65, spdStat: 90, desc: "패시브: 이동속도 +30% 🏃<br>스킬 1: 소드 마스터 - 4개의 검을 소환하여 순차 발사 (쿨타임 3.5초).<br>스킬 2: 얼굴 뽐내기 - 음파 발사.<br>궁극기: 신성력 - 지속 시간 동안만 충돌 데미지 활성화(5)." },
    ...
    { nickname: "가은", color: "#b339a3", name: "🖌️ 김가은", class: "util", hpStat: 45, atkStat: 60, spdStat: 95, desc: "패시브: 이동속도 +40% ⚡<br>스킬 1: 붓 슬래시.<br>스킬 2: 광역 붓질 - 적을 빨아들이고 폭발.<br>스킬 3: 표식 - 적중 시 체력 회복." },
    { nickname: "건우", color: "#e67e22", name: "🎙️ 김건우", class: "dealer", hpStat: 75, atkStat: 80, spdStat: 40, desc: "패시브: 이동속도 -20% 🐌<br>스킬 1: 하이퍼 파괴 레이저 (히트박스/데미지 상향).<br>스킬 2: 잠행 기습 - 땅으로 숨어 적 발밑 넉백." },
    ...
    { nickname: "민재", color: "#3498db", name: "🛡️ 김민재", class: "tanker", hpStat: 90, atkStat: 30, spdStat: 30, desc: "패시브: 받는 피해량 20% 영구 감소 🛡️<br>스킬 1: 방구 - 광역 어지러움.<br>스킬 2: 흡입 후 펀치 (쿨타임 4.5초)." },
    ...
    { nickname: "V티비", color: "#ff9ff3", name: "🔴 버튜버 김티비", class: "util", hpStat: 70, atkStat: 70, spdStat: 60, desc: "패시브: 4.5초마다 무작위 도네이션 스킬 발동.<br>도네이션: 먹기(20%), 사이먼도미닉(15%), 소드마스터(15%), 매력의개(15%), 저음레이저(15%), 신성력(10%), 힐(10%)." }
];
...
function initCharCards() {
    ...
    document.getElementById('grid-dealer').innerHTML = ''; document.getElementById('grid-util').innerHTML = '';
...
function goToMainHome() {
    ...
    p1Selected = null; p2Selected = null; p3Selected = null; balls = []; bullets = [];
    particles = []; groundSlamAreas = []; craters = []; soundwaves = []; textPopups = []; fanAttackPreviews = [];
    customGrabs = []; burrowPreviews = []; dogPuddles = []; activeDonations = []; earthquakes = []; parkSwords = [];
    simonProjectiles = []; dominicProjectiles = []; seqLaserBullets = []; darkBombThreats = []; inhaleZones = [];
    setGameSpeed(1.0);
...
function selectCharacter(char) {
    ...
    else if (!p2Selected) { p2Selected = char; playSelectImpact(char.color);
        if (gameMode === 2) { setTimeout(() => { startBattle(); }, 400); } else { document.getElementById('selection-status').innerText = "3P 대기중... 영웅을 고르세요"; } }
    else if (gameMode === 3 && !p3Selected) { p3Selected = char; playSelectImpact(char.color); setTimeout(() => { startBattle(); }, 400); }
}

function playSelectImpact(color) { flashBgColor = color; shakeTime = 12; shakeIntensity = 8; setTimeout(() => { flashBgColor = null; }, 120); }
...
function spawnImpactEffect(x, y, color, isWall = false) {
    if (isWall) { shakeTime = 10; shakeIntensity = 4; return; }
...
class Bullet {
    constructor(x, y, vx, vy, text, color, damage, pIndex, type = 'normal') {
        ...
        this.type = type;
        this.radius = type === 'ink_slash' ? 14 : (type === 'dog_charm' ? 16 : (type.startsWith('ink') || type === 'dark_assassinate' || type === 'dark_bomb' ? 12 : (type === 'laser_bullet' ? 6 : 16.2)));
        this.life = type === 'ink_slash' ? 45 : 240; this.angle = Math.atan2(vy, vx);
    }
    update() { this.x += this.vx * gameSpeed; this.y += this.vy * gameSpeed; this.life -= gameSpeed; }
    draw() {
        ...
        if (this.type === 'ink_slash') {
            ...
            ctx.fillStyle = this.color; ctx.font = "bold 13px sans-serif"; ctx.shadowBlur = 10; ctx.shadowColor = this.color; ctx.textAlign = "center";
...
class Ball {
    constructor(x, y, data, pIndex, isClone = false, master = null) {
        this.x = x; this.y = y; this.radius = data.radius; this.color = isClone ? "rgba(74, 21, 124, 0.65)" : data.color;
        this.fullName = data.name; this.nickname = isClone ? "분신" : data.nickname;
        this.maxHp = isClone ? 10 : data.maxHp; this.hp = this.maxHp; this.id = data.id; this.pIndex = pIndex;
        ...
        this.swallowedBall = null; this.swallowTimer = 0; this.eatDmgCount = 0;
        this.electricDuration = 0; this.burnDuration = 0; this.woundDuration = 0;
        this.slowDuration = 0; this.darkDebuffTimer = 0; this.isSlamCharging = false;
        this.parkCycleIdx = 0; this.divineTimer = 0; this.divinePauseTimer = 0;
        this.isFaceCharging = false; this.faceTargetAngle = 0; this.weakDebuffTimer = 0;
        this.isPhase2 = false; this.isTransforming = false; this.transformTimer = 0;
        this.tvBeamActive = false; this.tvBeamTimer = 0; this.tvBeamAngle = 0;
        this.isGrabbed = false; this.isMovingToCenter = false; this.wasHitByWindRush = false;
        this.isInkSpinning = false; this.inkSpinAngle = 0; this.markedTargetId = null;
        this.isInkPulling = false; this.inkPullTimer = 0; this.beamCycleId = 0;
        this.laserHitBeams = {};
        this.isBurrowed = false; this.isBurrowInAnimation = false; this.burrowAnimTimer = 45;
        this.burrowScale = 1; this.burrowTimer = 0; this.burrowTargetX = 0; this.burrowTargetY = 0;
        this.isGunwooEmerging = false; this.gunwooEmergeTimer = 0;
        this.voiceLaserActive = false; this.voiceLaserTimer = 0; this.voiceLaserAngleFixed = 0;
        this.laserFadeTimer = 0; this.laserSegments = null;
        this.deathSequenceTimer = 0; this.isDeadCompletely = false; this.dizzyTimer = 0;
        this.knockedByFart = false; this.knockedByPunch = false; this.knockTimer = 0;
        this.isTeleportStabbing = false; this.totalDamageTaken = 0;
        this.donationPendingSkill = null; this.donationTimer = 0;

        if (data.startAsPhase2) {
            this.isPhase2 = true; this.fullName = "🔥 각성 최해솔"; this.nickname = "최해솔"; this.color = "#ff4757";
        }

        // 차은우지성 전용 변수
        this.passiveAuraRadius = 55; this.auraCharmUpTimer = 0; this.auraHealTimer = 0; this.auraDmgTick = 0; this.selfHealTick = 0;
    }
    ...
    setNextSkillCooldown() {
        ...
        if (this.id === 0) { this.skillTimer = this.currentSkillStep === 1 ? 300 : 180; }
        else if (this.id === 1) { this.skillTimer = this.currentSkillStep === 1 ? 180 : 210; }
        else if (this.id === 2) {
            let step = [1, 2, 3, 1, 2, 1, 2, 3][this.parkCycleIdx];
            if (step === 1) this.skillTimer = 210; else if (step === 2) this.skillTimer = 360; else if (step === 3) this.skillTimer = 120;
        }
        else if (this.id === 3) {
            if (!this.isPhase2) { this.skillTimer = 99999; }
            else { if(this.currentSkillStep === 1) this.skillTimer = 240; else if(this.currentSkillStep === 2) this.skillTimer = 180; else this.skillTimer = 180; }
        }
        else if (this.id === 4) {
            if (this.currentSkillStep === 1) this.skillTimer = 210; else if (this.currentSkillStep === 2) this.skillTimer = 120; else this.skillTimer = 180;
        }
        else if (this.id === 5) { this.skillTimer = this.currentSkillStep === 1 ? 240 : 300; }
        else if (this.id === 6) {
            if (this.currentSkillStep === 1) this.skillTimer = 240; else if (this.currentSkillStep === 2) this.skillTimer = 240; else this.skillTimer = 120;
        }
        else if (this.id === 7) {
            if (this.currentSkillStep === 1) this.skillTimer = 210; else this.skillTimer = 270;
        }
        else if (this.id === 8) {
            if (this.currentSkillStep === 1) this.skillTimer = 210; else if (this.currentSkillStep === 2) this.skillTimer = 240; else this.skillTimer = 270;
        }
        else if (this.id === 9) {
            this.skillTimer = 270;
        }
        this.isCharging = false; this.isSlamCharging = false; this.isFaceCharging = false;
    }

    forceCancelSkill() {
        this.isFaceCharging = false; this.isSlamCharging = false; this.eatReady = false; this.tvBeamActive = false;
        this.isInkSpinning = false; this.isInkPulling = false; this.voiceLaserActive = false; this.isBurrowInAnimation = false;
        this.burrowScale = 1; this.divinePauseTimer = 0; this.isGunwooEmerging = false;
        this.donationPendingSkill = null; this.donationTimer = 0;
        fanAttackPreviews = fanAttackPreviews.filter(p => p.owner !== this);
        if (this.id === 2) { this.parkCycleIdx = (this.parkCycleIdx + 1) % 8; }
        else if (this.id === 4) { this.currentSkillStep = (this.currentSkillStep % 3) + 1; }
        else if (this.id === 6) { this.currentSkillStep = (this.currentSkillStep % 3) + 1; }
        else if (this.id === 7) { this.currentSkillStep = (this.currentSkillStep === 1) ? 2 : 1; }
        else if (this.id === 8) { this.currentSkillStep = (this.currentSkillStep % 3) + 1; }
        else { this.currentSkillStep = (this.currentSkillStep === 3) ? 1 : (this.currentSkillStep === 2 ? 3 : 2); }
        this.setNextSkillCooldown();
    }

    applyDamage(amount, attacker = null) {
        if(this.hp <= 0 || amount <= 0 || this.isTransforming || this.isMovingToCenter || this.isBurrowed) return;
        if (attacker && attacker.burnDuration > 0) amount *= 0.85; // 화상 데미지 15% 감소
        if (attacker && (attacker.id === 2 || attacker.id === 9) && attacker.divineTimer > 0) amount *= 1.5;
        if (this.id === 0) amount *= 0.9;
        if (this.id === 7) amount *= 0.8;
        ...
        if (this.hp <= 0) {
            ...
            if (this.id === 3 && !this.isPhase2) {
                this.hp = 1; this.isMovingToCenter = true; this.vx = 0; this.vy = 0; return;
            } else if (this.deathSequenceTimer === 0) {
                this.deathSequenceTimer = 120; this.vx = 0; this.vy = 0;
            }
        }

        balls.forEach(att => { if (att.id === 4 && att.markedTargetId === this.pIndex && att.hp > 0 && !att.isClone) { att.applyHeal(15); } });
    }
    ...
    executeDonationSkill() {
        ...
        if (skill === 'eat') {
            ...
            let launchAngle = target ? Math.atan2(target.y - this.y, target.x - this.x) : this.angle;
    ...
    update() {
        ...
        if (this.id === 99) {
            this.hp = this.maxHp;
            this.vx *= 0.85;
            this.vy *= 0.85; this.skillTimer = 99999;
        ...
        if (this.knockTimer > 0) {
            this.knockTimer -= gameSpeed;
            if (this.knockTimer <= 0) {
                this.knockedByFart = false;
                this.knockedByPunch = false;
            }
        }
            ...
            for (let b of balls) {
                if (b.hp > 0 && b.pIndex !== this.pIndex && !b.isSwallowed() && !b.isTransforming && !b.isMovingToCenter && !b.isBurrowed) {
                    ...
                    if (dist <= eatRange + b.radius) {
                        this.swallowedBall = b; this.eatReady = false; this.swallowTimer = 120; this.eatDmgCount = 0; b.forceCancelSkill(); break;
                    }
                }
            }
        }
        ...
        if (this.id === 5 && this.isBurrowInAnimation) {
            this.vx = 0; this.vy = 0; this.burrowAnimTimer -= gameSpeed; this.angle += 0.25 * gameSpeed;
            this.burrowScale = Math.max(0, this.burrowAnimTimer / 45);
            if (Math.random() < 0.4) particles.push(new Particle(this.x, this.y, "#855e42", 'burn'));
        ...
        if (this.isBurrowed) {
            ...
            if (this.isGunwooEmerging) {
                ...
                if (Math.random() < 0.2) particles.push(new Particle(this.burrowTargetX, this.burrowTargetY, "#e67e22", 'burn'));
                if (this.gunwooEmergeTimer <= 0) {
                    ...
                    burrowPreviews = burrowPreviews.filter(p => p.owner !== this);
                    flashBgColor = "rgba(230, 126, 34, 0.25)"; setTimeout(() => flashBgColor = null, 70);
                    shakeTime = 25; shakeIntensity = 15;
                    for(let i=0; i<15; i++) particles.push(new Particle(this.x, this.y, "#8B4513", 'sharp'));
                    for(let i=0; i<10; i++) particles.push(new Particle(this.x, this.y, "#A0522D", 'burn'));
                    for(let i=0; i<20; i++) particles.push(new Particle(this.x, this.y, "#e67e22", 'sharp'));
                    let dmg = 20; if (this.weakDebuffTimer > 0) dmg = 10;
                    balls.forEach(enemy => {
        ...
        if (this.isTransforming) {
            ...
            checkLaserCollision(this, this.tvBeamAngle, 5); this.transformTimer -= gameSpeed;
            if (this.transformTimer <= 0) {
                this.isTransforming = false; this.isPhase2 = true; this.maxHp = 200; this.hp = 200; this.bumpDmg = 0;
        ...
        if (this.isSwallowed()) {
            ...
            if (host) { this.x = host.x; this.y = host.y; } return;
        }
        ...
        if (this.id === 4 && this.isInkPulling) {
            ...
            if (Math.random() < 0.6) { let pAngle = Math.random() * Math.PI * 2;
                let pDist = Math.random() * 170;
        ...
        if (this.id === 99) {
            ...
            this.vx = 0; this.vy = 0;
        } else if (!(this.id === 4 && this.isInkPulling)) {
        ...
        if (hitWall) {
            ...
            if (mag > 0 && this.id !== 99) { this.vx = (this.vx / mag) * activeSpeed; this.vy = (this.vy / mag) * activeSpeed; }
            ...
            if (this.woundDuration > 0) {
                this.applyDamage(5, null);
                textPopups.push(new TextPopup(this.x, this.y, "상처!", false));
            } else if (this.burnDuration > 0) { this.applyDamage(5, null); }

            if (this.knockedByFart) {
                this.knockedByFart = false; this.knockTimer = 0; let minjae = balls.find(b => b.id === 7);
                this.applyDamage(20, minjae);
                this.dizzyTimer = 180;
                textPopups.push(new TextPopup(this.x, this.y - 15, "어지러움!", false)); createShockwave(this.x, this.y, 40, "#f1c40f");
            }
            if (this.knockedByPunch) {
                this.knockedByPunch = false;
                this.knockTimer = 0; let minjae = balls.find(b => b.id === 7); this.applyDamage(25, minjae); this.dizzyTimer = 180;
                textPopups.push(new TextPopup(this.x, this.y - 15, "어지러움!", false)); createShockwave(this.x, this.y, 50, "#e74c3c");
            }

            if (this.wasHitByWindRush) {
                this.wasHitByWindRush = false; let dmg = 5;
        ...
        if ((this.id === 0 || this.id === 9) && this.swallowedBall) {
            ...
            if (this.swallowTimer <= 60 && this.eatDmgCount === 0) { this.swallowedBall.applyDamage(15, this);
                this.eatDmgCount = 1;
            }
    ...
    triggerSkill() {
        ...
        if (this.id === 0) {
            if (this.currentSkillStep === 1) {
                ...
                setTimeout(() => { this.isSlamCharging = false; this.currentSkillStep = 1; this.setNextSkillCooldown(); }, 1000 / gameSpeed);
            }
        }
        else if (this.id === 1) {
            if (this.currentSkillStep === 1) {
                let target = this.getNearestEnemy(); let launchAngle = target ? Math.atan2(target.y - this.y, target.x - this.x) : this.angle;
                simonProjectiles.push({ x: this.x, y: this.y, vx: Math.cos(launchAngle) * 5.5, vy: Math.sin(launchAngle) * 5.5, text: "사이먼도미닉", color: "#2ed573", ownerIndex: this.pIndex, radius: 18, life: 120 });
            ...
            if (this.currentSkillStep === 1) {
                ...
                bullets.push(new Bullet(this.x, this.y, Math.cos(markAngle)*6, Math.sin(markAngle)*6, "", "#ff00ff", 0, this.pIndex, 'ink_mark'));
                this.currentSkillStep = 1; this.setNextSkillCooldown();
            }
        }
        else if (this.id === 5) {
            if (this.currentSkillStep === 1) {
                this.voiceLaserActive = true; this.voiceLaserTimer = 150;
                this.voiceLaserAngleFixed = Math.random() * Math.PI * 2;
            } else {
                this.isBurrowInAnimation = true; this.burrowAnimTimer = 45; this.burrowScale = 1;
                this.vx = 0; this.vy = 0;
            }
        }
        else if (this.id === 6) {
            if (this.currentSkillStep === 1) {
                ...
                let target = this.getNearestEnemy(); let shootAngle = target ? Math.atan2(target.y - this.y, target.x - this.x) : this.angle;
            ...
            if (this.currentSkillStep === 1) {
                groundSlamAreas.push({ timer: 60, owner: this, type: "fart" });
                this.currentSkillStep = 2; this.setNextSkillCooldown();
            } else {
                let target = this.getNearestEnemy(); let targetAngle = target ? Math.atan2(target.y - this.y, target.x - this.x) : this.angle;
                inhaleZones.push({ angle: targetAngle, timer: 90, owner: this });
                this.currentSkillStep = 1; this.setNextSkillCooldown();
            }
        }
        else if (this.id === 8) {
            ...
            if (rand < 0.20) {
                skillType = 'eat';
                text = "김민채님이 먹기를 후원하셨습니다!";
            } else if (rand < 0.35) {
                skillType = 'simon';
                text = "공병은님이 사이먼 도미닉을 후원하셨습니다!";
            } else if (rand < 0.50) {
                skillType = 'sword';
                text = "박지성님이 소드마스터를 후원하셨습니다!";
            } else if (rand < 0.65) {
                skillType = 'dog';
                text = "차은우지성님이 매력의 개를 후원하셨습니다!";
            } else if (rand < 0.80) {
                skillType = 'laser';
                text = "김건우님이 저음레이저를 후원하셨습니다!";
            } else if (rand < 0.90) {
                skillType = 'divine';
                text = "박지성님이 신성력을 후원하셨습니다!";
            } else {
                skillType = 'heal';
            ...
            this.donationPendingSkill = skillType;
            this.donationTimer = 30;
            this.setNextSkillCooldown();
        }
    }

    executeFaceExplosion() {
        let rangeRadius = size * 2; let baseAngle = this.faceTargetAngle;
        ...
        let dmg = this.weakDebuffTimer > 0 ? 15 : 30;
    ...
    draw() {
        ...
        if (this.darkDebuffTimer > 0 && this.hp > 0) {
            ctx.save();
            ctx.strokeStyle = "rgba(74, 21, 124, 0.95)"; ctx.lineWidth = 4; ctx.shadowBlur = 25; ctx.shadowColor = "#9b59b6";
            ctx.beginPath();
        ...
        if (this.hp <= 0 && this.deathSequenceTimer > 0) {
            ...
            let growth = (120 - this.deathSequenceTimer) * 0.12;
            activeRadius = Math.max(5, this.radius + pulse + growth);
        }
        ...
        ctx.fillStyle = this.color; ctx.shadowBlur = (this.hp <= 0 || this.isMovingToCenter) ? 28 : 18;
        ...
        if ((this.id === 0 || this.id === 9) && (this.eatReady || this.swallowedBall) && this.hp > 0) {
            ...
            ctx.strokeStyle = "rgba(255, 10, 10, 0.85)"; ctx.lineWidth = 3.5; ctx.shadowBlur = 18; ctx.shadowColor = "#ff0000";
            ctx.stroke();
        ...
        if (this.id === 0 && this.isSlamCharging) {
            ctx.save(); ctx.beginPath();
            ctx.arc(this.x, this.y, 172.5, 0, Math.PI * 2);
            ctx.fillStyle = "rgba(255, 71, 87, 0.08)"; ctx.strokeStyle = "rgba(255, 71, 87, 0.5)";
        ...
        if (this.burrowScale > 0.3) {
            ...
            ctx.font = "bold " + (12 * this.burrowScale) + "px sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle";
            ctx.shadowBlur = 5;
        ...
        if (this.id === 4 && this.hp > 0) {
            ...
            ctx.beginPath(); ctx.moveTo(30, -3); ctx.lineTo(44, 0); ctx.lineTo(30, 3); ctx.closePath(); ctx.fill(); ctx.restore();
        ...
        if (this.id === 4 && this.isInkSpinning && this.hp > 0) {
            ctx.save();
            ctx.strokeStyle = "rgba(255, 0, 255, 0.8)"; ctx.lineWidth = 4; ctx.shadowBlur = 15; ctx.shadowColor = "#ff00ff";
            ctx.beginPath();
...
function calculateVoiceLaser(owner) {
    ...
    let px = owner.x; let py = owner.y;
    let dx = Math.cos(currentAngle); let dy = Math.sin(currentAngle);
    owner.laserSegments = [{x: px, y: py}]; let bounces = 0;
    while (bounces < 4) {
        let intersectT = 99999;
        let wallType = '';
        if (dx > 0) { let t = (arenaRight - px) / dx; if (t < intersectT) { intersectT = t; wallType = 'V'; } }
        if (dx < 0) { let t = (arenaLeft - px) / dx; if (t < intersectT) { intersectT = t; wallType = 'V'; } }
        if (dy > 0) { let t = (arenaBottom - py) / dy; if (t < intersectT) { intersectT = t; wallType = 'H'; } }
        if (dy < 0) { let t = (arenaTop - py) / dy; if (t < intersectT) { intersectT = t; wallType = 'H'; } }
        px += dx * intersectT;
        py += dy * intersectT; owner.laserSegments.push({x: px, y: py}); bounces++;
        if (wallType === 'V') dx = -dx;
        else if (wallType === 'H') dy = -dy;
    }
    let dmgPerFrame = (owner.weakDebuffTimer > 0 ? 9 : 18) / 60 * gameSpeed;
    balls.forEach(enemy => {
        if (enemy.hp > 0 && enemy.pIndex !== owner.pIndex && !enemy.isSwallowed() && !enemy.isBurrowed && !enemy.isBurrowInAnimation && !enemy.isTransforming && !enemy.isMovingToCenter) {
            for (let i = 0; i < owner.laserSegments.length - 1; i++) {
                let p1 = owner.laserSegments[i]; let p2 = owner.laserSegments[i+1];
                let lDx = p2.x - p1.x; let lDy = p2.y - p1.y; let lineLenSq = lDx * lDx + lDy * lDy;
                ...
                let dist = Math.sqrt(Math.pow(enemy.x - closestX, 2) + Math.pow(enemy.y - closestY, 2));
                if (dist < enemy.radius + 20) { enemy.applyDamage(dmgPerFrame, owner); if (Math.random() < 0.2) particles.push(new Particle(closestX, closestY, "#e67e22", 'sharp')); break; }
            }
        }
    });
}
        ...
        if (sword.state === 'wait') {
            ...
            for(let b of balls) {
                if(b.hp > 0 && b.pIndex !== sword.owner.pIndex && !b.isSwallowed() && !b.isBurrowed && !b.isTransforming) {
                    ...
                    if(dist < b.radius + 8) {
                        b.applyDamage(sword.damage, sword.owner);
                        spawnImpactEffect(sword.x, sword.y, "#00ffff");
                        parkSwords.splice(i, 1);
                        hit = true;
                        break;
                    }
                }
            }
            if(!hit && (sword.x < arenaLeft || sword.x > arenaRight || sword.y < arenaTop || sword.y > arenaBottom)) {
                parkSwords.splice(i, 1);
            }
        }
    }
...
function handleCustomGrabs() {
    for (let i = customGrabs.length - 1; i >= 0; i--) {
        ...
        if (g.owner.hp <= 0 || g.owner.isTransforming || g.owner.isMovingToCenter || g.owner.isSwallowed()) {
            if (g.victim) g.victim.isGrabbed = false;
        ...
        if (g.state === 'launch') {
            ...
            if (g.x < arenaLeft || g.x > arenaRight || g.y < arenaTop || g.y > arenaBottom || g.length >= g.maxLength) {
                ...
                g.y = Math.max(arenaTop, Math.min(arenaBottom, g.y));
                g.state = 'pull';
            }
            balls.forEach(target => {
            ...
            if (dist > pullSpeed + 15) {
                ...
                if (g.victim) {
                    g.victim.x = g.x;
                    g.victim.y = g.y;
                    g.victim.x = Math.max(arenaLeft + g.victim.radius, Math.min(arenaRight - g.victim.radius, g.victim.x));
                ...
                if (g.victim) {
                    g.victim.isGrabbed = false;
                    let escapeAng = Math.random() * Math.PI * 2;
...
function handleInhaleZones() {
    if (isCountingDown) return;
    for (let i = inhaleZones.length - 1; i >= 0; i--) {
        let z = inhaleZones[i];
        z.timer -= gameSpeed;
        if (z.timer <= 0) { inhaleZones.splice(i, 1); continue; }
        let owner = z.owner; let zoneRemoved = false;
        balls.forEach(target => {
            if (zoneRemoved) return;
            if (target.hp > 0 && target.pIndex !== owner.pIndex && !target.isSwallowed() && !target.isBurrowed && !target.isTransforming) {
                let dx = target.x - owner.x, dy = target.y - owner.y; let dist = Math.sqrt(dx * dx + dy * dy);
                if (dist < size * 2) {
                    let angleToEnemy = Math.atan2(dy, dx); let diff = Math.atan2(Math.sin(angleToEnemy - z.angle), Math.cos(angleToEnemy - z.angle));
                    if (Math.abs(diff) <= (40 * Math.PI / 180)) {
                        target.vx -= Math.cos(angleToEnemy) * 0.6 * gameSpeed; target.vy -= Math.sin(angleToEnemy) * 0.6 * gameSpeed;
                        if (dist < owner.radius + target.radius + 15) {
                            let punchAngle = z.angle; target.vx = Math.cos(punchAngle) * 30; target.vy = Math.sin(punchAngle) * 30;
                            target.knockedByPunch = true; target.knockTimer = 60; createShockwave(owner.x, owner.y, 60, "#3498db"); zoneRemoved = true;
                        }
                    }
                }
            }
        });
        if (zoneRemoved) { inhaleZones.splice(i, 1); continue; }
    }
}
...
function handleDogPuddles() {
    ...
    for (let i = dogPuddles.length - 1; i >= 0; i--) {
        ...
        if (!p.tickTimer) p.tickTimer = 0;
        p.tickTimer -= gameSpeed;

        if (p.timer <= 0) {
            dogPuddles.splice(i, 1);
            continue;
        }
...
function handleGongSkills() {
    ...
    for (let i = simonProjectiles.length - 1; i >= 0; i--) {
        ...
        if (p.x < arenaLeft || p.x > arenaRight || p.y < arenaTop || p.y > arenaBottom || p.life <= 0) { simonProjectiles.splice(i, 1); continue; }
        let hit = false;
    ...
    for (let i = dominicProjectiles.length - 1; i >= 0; i--) {
        ...
        if (!d.isArrived) {
            ...
            if (d.life <= 0) {
                ...
                if (hitCount > 0) { let ownerHero = balls.find(ball => ball.pIndex === d.ownerIndex && !ball.isClone);
                    if (ownerHero && ownerHero.hp > 0) ownerHero.applyHeal(8 * hitCount); }
            }
    ...
    for (let i = groundSlamAreas.length - 1; i >= 0; i--) {
        ...
        if (area.timer <= 0) {
            if (area.type === 'slam') {
                let cracks = [];
                for(let c=0; c<14; c++){
                    let ang = (Math.PI*2/14)*c + (Math.random()-0.5)*0.5; let pts = []; let r = 0;
                    let cAng = ang;
                    while(r < 172.5) { r += 15 + Math.random()*20; cAng += (Math.random()-0.5)*0.8;
                        pts.push({x: Math.cos(cAng)*Math.min(r, 172.5), y: Math.sin(cAng)*Math.min(r, 172.5)}); }
                    cracks.push(pts);
                }
                ...
                balls.forEach(target => {
                    if (target.hp > 0 && target.pIndex !== area.owner.pIndex && !target.isSwallowed() && !target.isBurrowed && !target.isTransforming && !target.isMovingToCenter) {
                        ...
                        if (dist <= 172.5 + target.radius) { target.applyDamage(25, area.owner); target.slowDuration = 180; }
                    }
                });
                for(let k=0; k<20; k++) particles.push(new Particle(area.owner.x, area.owner.y, "#ff4757", 'sharp'));
            } else if (area.type === 'fart') {
                createShockwave(area.owner.x, area.owner.y, 172.5, "#3498db");
                shakeTime = 15; shakeIntensity = 10;
                balls.forEach(target => {
                    if (target.hp > 0 && target.pIndex !== area.owner.pIndex && !target.isSwallowed() && !target.isBurrowed && !target.isTransforming && !target.isMovingToCenter) {
                        let dist = Math.hypot(target.x - area.owner.x, target.y - area.owner.y);
                        if (dist <= 172.5 + target.radius) { target.applyKnockback(area.owner.x, area.owner.y, 15); target.knockedByFart = true; target.knockTimer = 60; }
                    }
                });
                for(let k=0; k<15; k++) particles.push(new Particle(area.owner.x, area.owner.y, "#3498db", 'burn'));
            }
        ...
        for (let j = 0; j < balls.length; j++) {
            ...
            if (dist < target.radius + b.radius) {
                ...
                if (b.type === 'dark_assassinate') {
                    target.applyDamage(b.damage, ownerHero); // 투사체 5뎀
                    if (ownerHero) {
                        ...
                        ownerHero.isTeleportStabbing = true;
                        ownerHero.vx = 0; ownerHero.vy = 0;
                        setTimeout(() => {
                            ownerHero.isTeleportStabbing = false;
                            if(ownerHero.hp > 0 && target.hp > 0 && !ownerHero.isDeadCompletely) {
                                target.applyDamage(16, ownerHero); // 찌르기 16뎀
                                ownerHero.applyHeal(5); // 힐 5
                                target.darkDebuffTimer = 180;
                                textPopups.push(new TextPopup(target.x, target.y - 20, "찌르기!", false));
                                createShockwave(target.x, target.y, 40, '#9b59b6');
                            }
                            let moveAng = Math.random() * Math.PI * 2;
                            ownerHero.vx = Math.cos(moveAng) * ownerHero.baseSpeed;
                            ownerHero.vy = Math.sin(moveAng) * ownerHero.baseSpeed;
                        }, 300 / gameSpeed);
                    }
                } else {
                    target.applyDamage(b.damage, ownerHero);
                    if (b.type === 'ink_mark') { if (ownerHero) ownerHero.markedTargetId = target.pIndex; }
                    else if (b.type === 'dark_bomb') {
                        createShockwave(b.x, b.y, 40, "#741b7c");
                        if (ownerHero) ownerHero.applyHeal(5); // 폭탄 힐량 5 적용
                    }
                    else if (b.type === 'dog_charm') {
                        dogPuddles.push({ x: target.x, y: target.y, timer: 240, ownerIndex: b.ownerIndex });
                    }
                }
...
function drawParkSwords() {
    parkSwords.forEach(s => {
        ...
        ctx.rotate(s.state === 'fire' ? s.angle : -Math.PI/4);
        ctx.fillStyle = "#00ffff"; ctx.shadowBlur = 10; ctx.shadowColor = "#00ffff";
        ctx.beginPath(); ctx.moveTo(15, 0); ctx.lineTo(0, -6); ctx.lineTo(-10, 0); ctx.lineTo(0, 6); ctx.fill();
        ctx.restore();
    });
}
...
function startBattle() {
    ...
    darkBombThreats = []; inhaleZones = []; earthquakes = []; parkSwords = []; dogPuddles = []; activeDonations = [];
    isCountingDown = true;
    countdownTimer = 3;
    const overlay = document.getElementById('countdown-overlay');
    overlay.innerText = countdownTimer; overlay.style.display = 'block';
...
function updateUI() {
    ...
    if(activeBalls[0]) {
        document.getElementById('p1-hp-bar').style.width = (activeBalls[0].hp / activeBalls[0].maxHp * 100) + '%'; document.getElementById('p1-hp-text').innerText = Math.ceil(activeBalls[0].hp) + '/' + activeBalls[0].maxHp;
        let cPct = activeBalls[0].isClone ? 0 : (1 - (activeBalls[0].skillTimer / (activeBalls[0].id === 0 ? (activeBalls[0].currentSkillStep===1?300:180) : (activeBalls[0].id===4 && activeBalls[0].currentSkillStep===3 ? 180 : (activeBalls[0].id===7 ? (activeBalls[0].currentSkillStep===1?210:270) : (activeBalls[0].id===8 ? (activeBalls[0].currentSkillStep===1?210:(activeBalls[0].currentSkillStep===2?240:270)) : (activeBalls[0].id===9 ? 270 : (activeBalls[0].id===2 && activeBalls[0].parkCycleIdx===1 ? 210 : 240))))))));
        cPct = Math.max(0, Math.min(1, cPct)); document.getElementById('p1-cool-bar').style.width = (cPct * 100) + '%';
    ...
    if(activeBalls[1]) {
        if (activeBalls[1].id === 99) {
            ...
            let cPct = 1 - (activeBalls[1].skillTimer / (activeBalls[1].id === 4 && activeBalls[1].currentSkillStep===3 ? 180 : (activeBalls[1].id===7 ? (activeBalls[1].currentSkillStep===1?210:270) : (activeBalls[1].id===8 ? (activeBalls[1].currentSkillStep===1?210:(activeBalls[1].currentSkillStep===2?240:270)) : (activeBalls[1].id===9 ? 270 : (activeBalls[1].id===2 && activeBalls[1].parkCycleIdx===1 ? 210 : 240))))));
    ...
    if(gameMode === 3 && activeBalls[2]) {
        ...
        let cPct = 1 - (activeBalls[2].skillTimer / (activeBalls[2].id === 4 && activeBalls[2].currentSkillStep===3 ? 180 : (activeBalls[2].id===7 ? (activeBalls[2].currentSkillStep===1?210:270) : (activeBalls[2].id===8 ? (activeBalls[2].currentSkillStep===1?210:(activeBalls[2].currentSkillStep===2?240:270)) : (activeBalls[2].id===9 ? 270 : (activeBalls[2].id===2 && activeBalls[2].parkCycleIdx===1 ? 210 : 240))))));
        cPct = Math.max(0, Math.min(1, cPct)); document.getElementById('p3-cool-bar').style.width = (cPct * 100) + '%';
...
function animate() {
    ...
    for (let i = earthquakes.length - 1; i >= 0; i--) {
        ...
        ctx.strokeStyle = `rgba(255, 71, 87, ${eq.life / 60})`;
        ctx.lineWidth = 3 + (eq.life / 20);
        ctx.shadowBlur = 10;
        ctx.shadowColor = "#ff4757";
        eq.cracks.forEach(crack => {
            ctx.beginPath();
            ctx.moveTo(0, 0);
            crack.forEach(pt => ctx.lineTo(pt.x, pt.y));
            ctx.stroke();
        });
        ctx.restore();
    }
    ...
    groundSlamAreas.forEach(area => {
        if (area.type === 'fart') { ctx.save(); ctx.beginPath(); ctx.arc(area.owner.x, area.owner.y, 172.5, 0, Math.PI * 2); ctx.fillStyle = "rgba(52, 152, 219, 0.15)"; ctx.fill(); ctx.strokeStyle = "rgba(52, 152, 219, 0.6)"; ctx.lineWidth = 1.5; ctx.setLineDash([5, 5]); ctx.stroke(); ctx.restore(); }
    });
    inhaleZones.forEach(z => {
        ctx.save(); ctx.fillStyle = "rgba(52, 152, 219, 0.25)"; ctx.strokeStyle = "rgba(52, 152, 219, 0.9)"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(z.owner.x, z.owner.y); ctx.arc(z.owner.x, z.owner.y, size * 2, z.angle - (40 * Math.PI/180), z.angle + (40 * Math.PI/180)); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.restore();
    });
    ...
    balls.forEach(b => {
        if ((b.id === 5 || b.id === 9) && b.voiceLaserActive && b.laserSegments) {
            ...
            ctx.strokeStyle = "rgba(255, 140, 0, 0.3)"; ctx.lineWidth = 26 + Math.sin(Date.now() / 40) * 6; ctx.shadowBlur = 24; ctx.shadowColor = "#ff8c00"; ctx.stroke(); ctx.strokeStyle = "rgba(255, 200, 0, 0.7)"; ctx.lineWidth = 12; ctx.shadowBlur = 12; ctx.stroke(); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 4; ctx.shadowBlur = 6; ctx.stroke(); ctx.restore();
        }

        if (!b.voiceLaserActive && b.laserFadeTimer > 0 && b.laserSegments) {
            ctx.save(); ctx.globalAlpha = b.laserFadeTimer / 30;
            if (b.id === 6) { ctx.strokeStyle = "#9b59b6"; ctx.lineWidth = 8; ctx.lineCap = "round"; 
                ctx.beginPath(); ctx.moveTo(b.laserSegments[0].x, b.laserSegments[0].y);
                ctx.lineTo(b.laserSegments[1].x, b.laserSegments[1].y);
                ctx.stroke(); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 3; ctx.stroke(); }
            else { ctx.strokeStyle = "rgba(230, 126, 34, 0.5)";
                ctx.lineWidth = 28;
                ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.beginPath(); ctx.moveTo(b.laserSegments[0].x, b.laserSegments[0].y); for(let k=1; k<b.laserSegments.length; k++) ctx.lineTo(b.laserSegments[k].x, b.laserSegments[k].y); ctx.stroke();
        ...
        if (b.tvBeamActive) {
            ctx.save(); ctx.strokeStyle = "rgba(255, 0, 0, 0.7)"; ctx.lineWidth = 5;
            ctx.shadowBlur = 10;
            ...
            ctx.stroke();
            ctx.strokeStyle = "rgba(255, 255, 255, 0.9)"; ctx.lineWidth = 2.5; ctx.shadowBlur = 0;
            ctx.beginPath();
            beamAngles.forEach(ang => { ctx.moveTo(b.x, b.y); ctx.lineTo(b.x + Math.cos(ang) * size * 1.5, b.y + Math.sin(ang) * size * 1.5); });
            ctx.stroke(); ctx.restore();
        }
    ...
    if (typeof dominicProjectiles !== 'undefined' && dominicProjectiles) {
        dominicProjectiles.forEach(d => {
            ...
            if (!d.isArrived) {
                ctx.save(); ctx.beginPath(); ctx.arc(d.tx || d.x, d.ty || d.y, 38.8125, 0, Math.PI * 2); ctx.strokeStyle = "rgba(0, 255, 255, 0.5)"; ctx.lineWidth = 2; ctx.setLineDash([4, 4]); ctx.stroke(); ctx.fillStyle = "rgba(0, 255, 255, 0.1)"; ctx.fill(); ctx.restore();
                ctx.save(); ctx.translate(d.x, d.y); ctx.shadowBlur = 10; ctx.shadowColor = d.color || "#00ffff"; ctx.fillStyle = d.color || "#00ffff"; ctx.font = "900 16px sans-serif"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(d.text || "", 0, 0); ctx.fillStyle = "#fff"; ctx.fillText(d.text || "", 0, 0); ctx.restore();
            }
        });
    }
    drawParkSwords();
    ...
    for (let i = activeDonations.length - 1; i >= 0; i--) {
        ...
        ctx.fillText(d.text, size/2, dYOffset);
        ctx.restore();
