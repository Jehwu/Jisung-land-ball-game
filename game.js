...
{ nickname: "V티비", color: "#ff9ff3", name: "🔴 버튜버 김티비", class: "util", hpStat: 75, atkStat: 70, spdStat: 60, desc: "패시브: 4.5초마다 무작위 도네이션 스킬 발동.<br>도네이션: 모든 스킬 동일 확률(약 14.28%) 발동." },
{ nickname: "김민제미나이", color: "#00ff00", name: "💻 김민제미나이", class: "dealer", hpStat: 60, atkStat: 85, spdStat: 70, desc: "스킬 1: 레이저 펄스 - 적에게 화상을 입히는 일직선 레이저 빔 발사.<br>스킬 2: 데이터 폭발 - 적을 2초간 빨아들이는 구역을 생성 후 폭발(20뎀).<br>스킬 3: 해킹 - 적에게 링크를 연결하여 2초 뒤 폭발(20뎀)과 함께 4초간 해킹 디버프를 부여합니다." }
];
...
seqLaserBullets = []; darkBombThreats = [];
setGameSpeed(1.0);
...
this.x = x;
this.y = y; this.radius = data.radius; this.color = isClone ? "rgba(74, 21, 124, 0.65)" : data.color;
this.fullName = data.name;
this.nickname = isClone ? "분신" : data.nickname;
...
this.nickname = "최해솔"; this.color = "#ff4757";
}

// 차은우지성 전용 변수
this.passiveAuraRadius = 55; this.auraCharmUpTimer = 0;
this.auraHealTimer = 0;
this.auraDmgTick = 0; this.selfHealTick = 0;
}
...
if (step === 1) this.skillTimer = 210; else if (step === 2) this.skillTimer = 360;
else if (step === 3) this.skillTimer = 120;
}
else if (this.id === 3) {
...
if (this.currentSkillStep === 1) this.skillTimer = 210; else if (this.currentSkillStep === 2) this.skillTimer = 120;
else this.skillTimer = 180;
}
else if (this.id === 5) { this.skillTimer = this.currentSkillStep === 1 ? 240 : 300; }
else if (this.id === 6) {
if (this.currentSkillStep === 1) this.skillTimer = 240; else if (this.currentSkillStep === 2) this.skillTimer = 240;
else this.skillTimer = 120;
}
else if (this.id === 8) {
if (this.currentSkillStep === 1) this.skillTimer = 210;
else if (this.currentSkillStep === 2) this.skillTimer = 240; else this.skillTimer = 270;
}
else if (this.id === 9) {
this.skillTimer = 270;
}
else if (this.id === 10) {
if (this.currentSkillStep === 1) this.skillTimer = 180;
else if (this.currentSkillStep === 2) this.skillTimer = 240;
else this.skillTimer = 240;
}
...
this.applyDamage(dmg, bjs); shakeTime = 14; shakeIntensity = 8;
...
else { skillType = 'heal'; text = "메르시님이 좋은 힐을 후원하셨습니다!"; }
...
let rX = arenaLeft + 90 + Math.random() * (arenaRight - arenaLeft - 180);
let rY = arenaTop + 90 + Math.random() * (arenaBottom - arenaTop - 180);
...
let px = owner.x; let py = owner.y; let dx = Math.cos(currentAngle); let dy = Math.sin(currentAngle);
...
if (g.victim) g.victim.isGrabbed = false; customGrabs.splice(i, 1); continue;
}
if (g.state === 'launch') {
...
if (dist <= 90 + enemy.radius && dist > 5) {
enemy.x += (d.x - enemy.x) * 0.05 * gameSpeed; enemy.y += (d.y - enemy.y) * 0.05 * gameSpeed;
}
}
});
if (Math.random() < 0.4) particles.push(new Particle(d.x + (Math.random()-0.5)*90, d.y + (Math.random()-0.5)*90, "#00ff00", 'sharp'));
if (d.explosionTimer <= 0) {
let ownerHero = balls.find(ball => ball.pIndex === d.ownerIndex && !ball.isClone);
balls.forEach(enemy => {
if (enemy.hp > 0 && enemy.pIndex !== d.ownerIndex && Math.hypot(enemy.x - d.x, enemy.y - d.y) <= 90 + enemy.radius) {
enemy.applyDamage(20, ownerHero);
}
});
createShockwave(d.x, d.y, 90, "#00ff00");
...
darkBombThreats = []; earthquakes = []; parkSwords = []; dogPuddles = []; activeDonations = [];
isCountingDown = true;
countdownTimer = 3;
...
let cPct = 1 - (activeBalls[1].skillTimer / (activeBalls[1].id === 4 && activeBalls[1].currentSkillStep===3 ? 180 : (activeBalls[1].id===8 ? (activeBalls[1].currentSkillStep===1?210:(activeBalls[1].currentSkillStep===2?240:270)) : (activeBalls[1].id===9 ? 270 : (activeBalls[1].id===10 ? (activeBalls[1].currentSkillStep===1?180:240) : (activeBalls[1].id===2 && activeBalls[1].parkCycleIdx===1 ? 210 : 240))))));
...
let cPct = 1 - (activeBalls[2].skillTimer / (activeBalls[2].id === 4 && activeBalls[2].currentSkillStep===3 ? 180 : (activeBalls[2].id===8 ? (activeBalls[2].currentSkillStep===1?210:(activeBalls[2].currentSkillStep===2?240:270)) : (activeBalls[2].id===9 ? 270 : (activeBalls[2].id===10 ? (activeBalls[2].currentSkillStep===1?180:240) : (activeBalls[2].id===2 && activeBalls[2].parkCycleIdx===1 ? 210 : 240))))));
...
ctx.beginPath(); 
ctx.moveTo(b.laserSegments[0].x, b.laserSegments[0].y); ctx.lineTo(b.laserSegments[1].x, b.laserSegments[1].y); ctx.stroke(); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 3; ctx.stroke();
}
else { ctx.strokeStyle = "rgba(230, 126, 34, 0.5)";
ctx.lineWidth = 28; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.beginPath(); ctx.moveTo(b.laserSegments[0].x, b.laserSegments[0].y);
...
let drawRadius = d.type === 'data_grenade' ? 90 : 38.8125;
...
ctx.strokeStyle = rangeColor; ctx.lineWidth = 2; ctx.setLineDash([4, 4]); ctx.stroke();
